...
def sanitize_control_dic(control_dic):
    ...
    """
    control_dic['cmd_motor_speeds'] = np.asarray(control_dic['cmd_motor_speeds'], float).ravel()
    control_dic['cmd_moment'] = np.asarray(control_dic['cmd_moment'], float).ravel()
    control_dic['cmd_q'] = np.asarray(control_dic['cmd_q'], float).ravel()
    return control_dic
...
class Quadrotor(object):
    ...
    def statedot(self, state, cmd_rotor_speeds, curr_time, t_step, thrust_scale=None, fault_time=0.0, fault_profile="normal"):
        ...
        if curr_time >= fault_time:
            overall_eff = np.mean([self.inject_fault(curr_time, fault_time, ts, fault_profile) for ts in thrust_scale])
            overall_severity = 1.0 - overall_eff
            disturbance = np.array([
                0.02 * overall_severity * (np.random.rand() - 0.5),
                0.02 * overall_severity * (np.random.rand() - 0.5),
                0.05 * overall_severity * (np.random.rand() - 0.5)
            ])
            TM[1:4] += disturbance
